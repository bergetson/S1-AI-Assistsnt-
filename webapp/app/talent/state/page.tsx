...
/**
 * useSearchParams reads the live browser URL, which is fine under a static
 * export — but Next requires it to sit inside a Suspense boundary or the
 * prerender fails. Hence the split: this component owns the hook, the default
 * export supplies the boundary.
 */
function StateAnalytics() {
  ...
  // ?tab= lets a dashboard tile or an action land the user on the right tab
  // instead of on the overview with a note to go looking.
  const params = useSearchParams()
  const requested = params.get('tab')
  const [tab, setTab] = useState<Tab>(
    requested && TAB_IDS.has(requested) ? (requested as Tab) : 'overview')
